...
# Read time series from all the files in folder.
# Also get the one-hot class for each.
#--------------------------------------------------
def read_series( path, columns):
    files = os.listdir( path)
    csvfiles  = [f for f in files if f.endswith( '.csv') and f[0] != '.']
    jsonfiles = [os.path.splitext(f)[0] + '.json' for f in csvfiles]
    res_series = []
    res_class = []
    for idx,csvfile in enumerate(csvfiles):
        jsonfile = jsonfiles[idx]
        series = csv2dict( open( path + '/' + csvfile,'r').read())[0]
        series = [ [ d[c] for c in columns ] for d in series ]
        cclass = json.loads( open( path + '/' + jsonfile,'r').read())
        res_series.append( series)
        res_class.append( cclass['class'])

    res_class = onehot( res_class)
    return np.array( res_series, float), np.array( res_class, float)
