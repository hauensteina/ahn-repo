...
def main():
    ...
    for i in range(MAX_STEPS):
        ...
        hpreact_fast = ( bngain * (hprebn - hprebn.mean(0, keepdim=True)) / 
                        torch.sqrt(hprebn.var(0, unbiased=True, keepdim=True) + 1e-5) ) + bnbias
        #print('maxdiff hpreact', (hpreact - hpreact_fast).abs().max().item())
        # Non-linearity
        h = torch.tanh(hpreact_fast) # (train_set_size, hidden_size)
        ...
        for p in parameters:
            p.grad = None   
        for t in [logits, h, hpreact_fast, bnraw, 
        ...
        cmp('W2', dW2, W2)
        cmp('b2', db2, b2)
        dhpreact_fast = dh * (1 - h ** 2)
        cmp('hpreact_fast', dhpreact_fast, hpreact_fast)

        dhpreact = dh * (1 - h ** 2)
        #cmp('hpreact', dhpreact, hpreact)
        cmp('hpreact', dhpreact, hpreact_fast)
        ...
        #dhprebn = dbndiff + dbnmeani / n # slow version needed all the gradients above
        # All in one go
