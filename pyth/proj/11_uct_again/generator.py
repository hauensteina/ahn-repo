...
#tf.config.experimental.set_visible_devices([], device_type='GPU') # Hide GPU. CPU is faster on unbatched net runs.
...
def main():
    ...
    model = ShiftModel( size=3, mode='v')
    gen = Generator( model)
    gen.run()
...
class Generator:
    ...
    def __init__( self, model,
        ...
        self.modelfile = modelfile # Separate training process stores updated weights here
        ...
        self.modeltime = None # datetime.utcnow()

    def run( self):
        #print( '>>> Looking for new weights %s every %d games' % (self.modelfile, self.chunksize))
