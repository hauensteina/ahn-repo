...
// Log integer vector with line breaks and alignment
//-----------------------------------------------------
inline void log_vec_i( NSString *str, std::vector<int> v)
{
...
// Log float vector with line breaks and alignment
//-----------------------------------------------------
inline void log_vec_f( NSString *str, std::vector<float> v)
{
    NSString *msg = nsprintf( @"%@", str);
    ISLOOP (v) {
        if (i%19 == 0) msg = nscat( msg, @"\n");
