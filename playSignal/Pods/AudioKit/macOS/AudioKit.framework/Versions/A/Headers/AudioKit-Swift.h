...
SWIFT_CLASS_PROPERTY(@property (nonatomic, class) BOOL disableAudioSessionDeactivationOnStop;)
+ (BOOL)disableAudioSessionDeactivationOnStop SWIFT_WARN_UNUSED_RESULT;
+ (void)setDisableAudioSessionDeactivationOnStop:(BOOL)value;
...
typedef SWIFT_ENUM(NSInteger, BufferLength, closed) {
/// Shortest
...
typedef SWIFT_ENUM(NSInteger, RampType, closed) {
  RampTypeLinear = 0,
...
typedef SWIFT_ENUM(NSInteger, AKTableType, closed) {
...
/// \param completionHandler Callback to run when the sample playback is completed
///
- (nonnull instancetype)initWithFile:(AKAudioFile * _Nullable)file startPoint:(uint32_t)startPoint endPoint:(uint32_t)endPoint rate:(double)rate volume:(double)volume maximumSamples:(NSInteger)maximumSamples completionHandler:(void (^ _Nonnull)(void))completionHandler loadCompletionHandler:(void (^ _Nonnull)(void))loadCompletionHandler OBJC_DESIGNATED_INITIALIZER;
/// Function to start, play, or activate the node, all do the same thing
- (void)start;
/// Function to stop or bypass the node, both are equivalent
- (void)stop;
